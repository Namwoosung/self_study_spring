...
public class MemberRepositoryTest {
    ...
    public void findMemberLazy(){
        ...
        //when
        List<Member> members = memberRepository.findAll(); //member만 조회(team은 Lazy이므로 조회 x)

        //then
        for (Member member : members) {
            System.out.println("member.getUsername() = " + member.getUsername());
            System.out.println("member.getTeam().getName() = " + member.getTeam().getName()); //Lazy로딩, Team을 각각 조회
        }
    }
    ...
    public void queryByExample(){
        ...
        Example<Member> example = Example.of(member, matcher);//엔티티를 넘겨서 example로 사용

        List<Member> result = memberRepository.findAll(example);//스프링 데이터에서는 example을 받는 메서드도 지원
